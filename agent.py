...
class DQN(object):
    ...
    def choose_action(self, state,dis1,dis2, mean_action):    # 玻尔兹曼策略
        #action_probs_numes = []
        #denom = 0

        # list_dis = []
        # list_dis.append(dis1)
        # list_dis.append(dis2)
        states = np.append(state,[dis1,dis2])
        input_state = np.append(states, mean_action)  # input_state是一维向量

        # input_state = input_state + [0]*5
        # input_state.extend(dis)
    ...
    def store_transition(self, s, dis1, dis2, a, r, s_, dis1_, dis2_, mean_a):                                    # 定义记忆存储函数 (这里输入为一个transition)
        states = np.append(s, [dis1, dis2])
        states_ = np.append(s_, [dis1_, dis2_])
        transition = np.hstack((states, [a, r], states_, [mean_a]))                                 # 在水平方向上拼接数组
        # 如果记忆库满了，便覆盖旧的数据
    ...
    def learn(self):                                                            # 定义学习函数(记忆库已满后便开始学习)
        ...
        q_next = self.target_net(target_input).detach()
