...
class Net(nn.Module):
    def __init__(self, agent_id, args):                                                         # 定义Net的一系列属性
        ...
        self.input_size = args.state_dim + args.n_actions
...
class DQN(object):
    def __init__(self,  agent_id, args):  # 定义DQN的一系列属性
        ...
        self.memory = np.zeros((self.buffer_size, self.state_dim * 2 + self.action_dim + self.n_actions + 1))  # 初始化记忆库，一行代表一个transition
